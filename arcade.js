(async function () {
    ...
    set('dateLabel', today.toLocaleDateString('en-US', {
        weekday: 'short',
        month: 'short',
        day: 'numeric'
    }).toUpperCase());
    ...
    // The filter state is mirrored into the query string so a copied URL
    // reproduces exactly what the sender was looking at. `state` stays the
    // single source of truth; the URL is a projection of it, written on every
    // apply() and read back on load + popstate.
    const TYPES = ['all', 'tool', 'game'];
    const FILTERS = ['featured', 'ai', 'new'];
    const SORTS = ['newest', 'updated', 'az', 'feat'];
    const DEFAULTS = {type: 'all', filter: null, tag: null, sort: 'newest', search: ''};

    // Guard against feedback loops: while we're restoring from the URL we must
    // not write back to it (that would clobber the entry we just navigated to).
    let restoringFromURL = false;
    // Search fires on every keystroke — those get replaceState so the back
    // button doesn't have to walk through each character. Discrete actions
    // (clicking a tag, a type, a sort pill) get a real history entry.
    let urlWriteMode = 'push';
    ...
    function readStateFromURL() {
        ...
        state.search = p.get('q') || DEFAULTS.search;
        state.type = pick(p.get('type'), TYPES, DEFAULTS.type);
        state.filter = pick(p.get('filter'), FILTERS, DEFAULTS.filter);
    ...
    function syncURL() {
        ...
        const url = location.pathname + stateToQuery() + location.hash;
        // Don't spam identical entries (e.g. re-clicking the active sort pill).
        if (url === location.pathname + location.search + location.hash) return;
        try {
            history[urlWriteMode === 'replace' ? 'replaceState' : 'pushState']({arcadeFilters: true}, '', url);
        } catch (e) { /* file:// or sandboxed — filtering still works, just not shareable */
        }
    }
    ...
    function reflectSelection() {
        ...
        document.querySelectorAll('.pill-btn[data-sort]').forEach(b => {
            b.classList.toggle('on', b.dataset.sort === state.sort);
        });
    }

    // Hydrate from the incoming URL before anything renders, so a shared link
    // never flashes the unfiltered grid first.
    const hadURLState = readStateFromURL();
    buildFilterLists();          // reflectSelection() inside picks up the state
    ...
    function onSearchInput(e) {
        ...
        updateBurgerDot();
        urlWriteMode = 'replace';   // one history entry per search session, not per keystroke
        apply();
    ...
    function apply() {
        // Every mutation path funnels through here, so this is the one place the
        // URL needs to be kept in step.
        syncURL();
        urlWriteMode = 'push';
    ...
    window.addEventListener('resize', () => {
        ...
        raf = requestAnimationFrame(reAugmentAll);
    });
    ...
    const lucky = () => {
        ...
        window.open(href(x), '_blank')
    };
    const lb1 = document.getElementById('luckyBtn');
    if (lb1) lb1.addEventListener('click', lucky);
    const lb2 = document.getElementById('luckyBtn2');
    if (lb2) lb2.addEventListener('click', lucky);
    const lbM = document.getElementById('luckyBtnM');
    if (lbM) lbM.addEventListener('click', () => {
        lucky();
        closeDrawer({skipRefocus: true});
    });
    ...
    // Back/forward: re-read the URL and rebuild the view from it.
    window.addEventListener('popstate', () => {
        restoringFromURL = true;
        readStateFromURL();
        syncSearchInputs(state.search, null);
        reflectSelection();
        updateBurgerDot();
        apply();
    ...
    // First paint: seed the inputs from the URL and normalise the address bar
    // (replaceState, so we don't push a bogus entry on top of the landing page).
    syncSearchInputs(state.search, null);
    urlWriteMode = 'replace';
    apply();
    updateBurgerDot();

    // A shared link points at a filtered result set — land the reader on it.
    if (hadURLState && !location.hash) {
    ...
    const accents = [{n: 'Amber', c: '#ffc857'}, {n: 'Coin-op Red', c: '#ff4d2e'}, {
        n: 'Neon Cyan',
        c: '#4ee1d2'
    }, {n: 'Lime', c: '#b4f04a'}, {n: 'Magenta', c: '#ff3ca1'}];
    ...
    swA.addEventListener('click', e => {
        const sw = e.target.closest('.sw');
        if (!sw) return;
        TWEAKS.accent = sw.dataset.c;
        [...swA.children].forEach(x => x.classList.remove('on'));
        sw.classList.add('on');
        applyTweaks(TWEAKS);
        sendEdits({accent: TWEAKS.accent})
    });
    document.getElementById('scanSel').value = TWEAKS.scanlines;
    document.getElementById('scanSel').addEventListener('change', e => {
        TWEAKS.scanlines = e.target.value;
        applyTweaks(TWEAKS);
        sendEdits({scanlines: TWEAKS.scanlines})
    });
    ...
    document.getElementById('density').addEventListener('change', e => {
        ...
        applyTweaks(TWEAKS);
        sendEdits({density: TWEAKS.density})
    });
    document.getElementById('hstyle').value = TWEAKS.hstyle;
    document.getElementById('hstyle').addEventListener('change', e => {
        TWEAKS.hstyle = e.target.value;
        applyTweaks(TWEAKS);
        sendEdits({hstyle: TWEAKS.hstyle})
    });
    ...
    window.addEventListener('message', e => {
        const d = e.data || {};
        if (d.type === '__activate_edit_mode') document.getElementById('tweaks').classList.add('vis');
        if (d.type === '__deactivate_edit_mode') document.getElementById('tweaks').classList.remove('vis')
    });
    try {
