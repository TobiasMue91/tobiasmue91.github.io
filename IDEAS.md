...
standing in the way. The first build of it failed a cold read badly, and the failure is worth recording: the mechanic was fine, the teaching was not. It opened with 250 words of prose and then a mission carrying seven systems at once (three spread relays, two enemy types, walls, chasms, a timer, a bonus objective, a power meter, an upgrade screen), with enemies drawn as abstract shapes you had to match against a legend and a forecast written in arithmetic. The fix was to teach by playing: five hand-built one-move boards, each carrying a single idea, each verified by exhaustive search to be solvable, not solvable by doing nothing, and to have something of yours visibly at stake — that last check caught a lesson where nothing was actually threatened. Alongside that, three readability changes that would have been worth making anyway: every piece carries its own name so nothing needs looking up, a lethal hit reads DIES or DESTROYED instead of a number you have to subtract yourself, and a sentence under the board says what ending the turn would do ("End the turn now and one of theirs dies") — which is also the thing that makes hovering a shove legible. The opening sector then drops to two relays and four rounds, and the objectives, refits and power economy switch on one at a time. Still open on the tactics side: **line-routing** (the Mini Metro shape is still nothing
