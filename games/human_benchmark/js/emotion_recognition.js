...
const game={score:0,lives:3,currentEmotion:'',lastEmotion:'',bestScore:null};
const els={start:$('#emo-start'),imageArea:$('#emo-image-area'),image:$('#emo-image'),optionsArea:$('#emo-options-area'),scoreDisplay:$('#emo-score'),livesDisplay:$('#emo-lives'),bestDisplay:$('#emo-best')};
...
const updateUI=()=>{els.scoreDisplay.text(game.score);els.livesDisplay.text(game.lives);};
...
const gameOver=()=>{const isNewRecord=saveScore('Emotion Recognition',game.score);if(isNewRecord)game.bestScore=game.score;HB.showModal({title:isNewRecord?'🎉 New Personal Best!':'Game Over',message:`You correctly identified <strong>${game.score}</strong> emotions!${game.score>=20?'<br>🏆 Emotion expert!':''}`,score:`${game.score} correct`,isNewRecord,onRetry:reset,onHome:HB.goHome,icon:'😊'});};
const reset=()=>{game.score=0;game.lives=3;game.currentEmotion='';game.lastEmotion='';els.start.show();els.imageArea.addClass('hidden');els.optionsArea.addClass('hidden');updateUI();loadBest();};
const start=()=>{els.start.hide();els.imageArea.removeClass('hidden');els.optionsArea.removeClass('hidden');showRandomEmotion();};
